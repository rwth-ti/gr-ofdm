    ...
    long station_id)
{
  return ofdm_corba_rxbaseband_sink_sptr(new ofdm_corba_rxbaseband_sink(sevent_channel,
      snameservice_ip, snameservice_port, vlen, vlen_sym, station_id));
}
...
ofdm_corba_rxbaseband_sink::ofdm_corba_rxbaseband_sink(std::string sevent_channel,
  ...
  : gr_sync_block("corba_rxbaseband_sink",
      gr_make_io_signature2(2, 2, sizeof(float)*vlen,sizeof(float)*vlen_sym),
    ...
    gr_vector_void_star &output_items)
{
  ...
  const float *timing = static_cast<const float*>(input_items[1]);

  ofdm_ti::baseband_rx rxbaseband;
  ofdm_ti::float_sequence corba_bband(d_vlen);
  corba_bband.length(d_vlen);
  ofdm_ti::float_sequence corba_timing(d_vlen_sym);
  corba_timing.length(d_vlen_sym);

#ifdef KEEPLAST

  bband += (noutput_items-1)*d_vlen;
  timing += (noutput_items-1)*d_vlen_sym;
  //id += (noutput_items-1);
  //frlos += (noutput_items-1);

#else

  for(int j = 0; j < noutput_items; ++j, /*++id,*/ bband+=d_vlen, timing+=d_vlen_sym)
...

  {
    ...
    for(int i = 0; i < d_vlen_sym; ++i){
      corba_timing[i] = static_cast<CORBA::Float>(timing[i]);
        }
    //rxbaseband.rx_id = static_cast<CORBA::ULong>(*id);
    rxbaseband.bband = corba_bband;
    rxbaseband.timing_metric = corba_timing;
