...
/* $Id: ofdm_dominiks_sync_01.cc 1044 2009-07-02 12:50:54Z auras $ */
...
/*!
 *
 * in 0: received samples
 * in 1: Schmidl & Cox metric
 * in 2: cyclic prefix autorcorrelation / guard interval metric
 *  // in 3: control feedback stream, shift window
 *
 * out 0: ofdm blocks
 * out 1: frame trigger (1 == preamble detected)
 *
 * @param vlen
 * @param cplen
 * @return
...
{
  return ofdm_dominiks_sync_01::create( vlen, cplen );
}
...
create( int vlen, int cplen )
{
  try
  {
    ofdm_dominiks_sync_01_sptr tmp(
      new ofdm_dominiks_sync_01( vlen, cplen ) );
  ...
  catch ( ... )
  {
    std::cerr << "[ERROR] Caught exception at creation of "
              << "ofdm_dominiks_sync_01" << std::endl;
    throw;
  } // catch ( ... )
}
