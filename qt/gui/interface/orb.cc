...
public:
...

  {
    ...
    try {
      ...
      CORBA::String_var ior = d_orb->object_to_string( context_obj.in() );
...
orb_wrapper::resolve( std::string name )
{
  ...
  try
  {
    std::vector< std::string > ctx_decode;

    int pos = -1;
    while( ( pos = name.find( "." ) ) != std::string::npos )
    {
      ctx_decode.push_back( name.substr( 0, pos ) );
      name = name.substr( pos+1 );
    }

    CosNaming::Name cosname( ctx_decode.size() + 1 );
    cosname.length( ctx_decode.size() + 1 );
    for( int i = 0; i < ctx_decode.size(); ++i )
    {
      cosname[i].id = CORBA::string_dup( ctx_decode[i].c_str() );
      std::cout << "ctx: " << ctx_decode[i] << std::endl;
    }
    if( ctx_decode.size() == 0 )
      cosname[ ctx_decode.size() ].id = CORBA::string_dup( name.c_str() );
    else
    {
      cosname[ ctx_decode.size()-1 ].kind = CORBA::string_dup( name.c_str() );
      cosname.length( ctx_decode.size() );
    }

    std::cout << "final: " << name << std::endl;
